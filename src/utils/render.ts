...
import { loadProfile } from "./storages.ts";
...
export function renderPostCard(post: Post): HTMLElement {
  ...
  if (isOwnPost) {
    ...
    profileLink.href = `../profile/index.html?name=${encodeURIComponent(authorName)}`;
  ...
  authorLink.href = `../profile/index.html?name=${encodeURIComponent(authorName)}`;

  const authorAvatar = document.createElement("img");
  authorAvatar.className = "author-avatar";
  authorAvatar.src =
    post.author?.avatar?.url ??
    `https://ui-avatars.com/api/?name=${encodeURIComponent(authorName)}&background=3f5b4a&color=ffffff`;
